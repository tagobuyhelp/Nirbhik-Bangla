const bn = {
  // Navigation
  nav: {
    home: 'প্রচ্ছদ',
    live_tv: 'লাইভ টিভি',
    breaking: 'ব্রেকিং',
    trending: 'ট্রেন্ডিং',
    search: 'অনুসন্ধান করুন...',
    language: 'ভাষা',
    theme: 'থিম',
    dark_mode: 'ডার্ক মোড',
    light_mode: 'লাইট মোড',
    login: 'প্রবেশ করুন',
    dashboard: 'ড্যাশবোর্ড',
  },

  // Categories (generic)
  categories: {
    politics: 'রাজনীতি',
    sports: 'খেলাধুলা',
    entertainment: 'বিনোদন',
    business: 'ব্যবসা',
    technology: 'প্রযুক্তি',
    international: 'আন্তর্জাতিক',
    national: 'জাতীয়',
    state: 'রাজ্য',
    lifestyle: 'জীবনযাপন',
    education: 'শিক্ষা',
    health: 'স্বাস্থ্য',
    opinion: 'মতামত',
    religion: 'ধর্ম',
    crime: 'অপরাধ',
    science: 'বিজ্ঞান',
  },

  // Category names by slug (for homepage category grids)
  category_names: {
    'paschim-bardhaman': 'পশ্চিম বর্ধমান',
    'asansol': 'আসানসোল',
    'durgapur': 'দুর্গাপুর',
    'rajya': 'রাজ্য',
    'desh': 'দেশ',
    'biswa': 'বিশ্ব',
    'khela': 'খেলা',
    'binodon': 'বিনোদন',
    'lifestyle': 'লাইফস্টাইল',
    'projukti': 'প্রযুক্তি',
    'tech': 'প্রযুক্তি',
    'rajniti': 'রাজনীতি',
    'arthaniti': 'অর্থনীতি',
    'video': 'ভিডিও',
    'breaking': 'ব্রেকিং নিউজ',
    'latest': 'সর্বশেষ খবর',
    'popular': 'জনপ্রিয় খবর',
    'special-report': 'বিশেষ প্রতিবেদন',
    'all': 'সব বিভাগ',
  },

  // Homepage Sections
  home: {
    hero_title: 'নির্ভীক বাংলা',
    hero_subtitle: 'নিরপেক্ষ ও নির্ভীক সাংবাদিকতা',
    breaking_news: 'ব্রেকিং নিউজ',
    trending_now: 'ট্রেন্ডিং',
    latest_news: 'সর্বশেষ খবর',
    featured: 'বিশেষ প্রতিবেদন',
    video_news: 'ভিডিও খবর',
    photo_gallery: 'ফটো গ্যালারি',
    live_now: 'এখন সরাসরি',
    read_more: 'বিস্তারিত পড়ুন',
    view_all: 'সবগুলো দেখুন',
    more_news: 'আরও খবর',
    poll_title: 'আজকের জনমত',
    poll_vote: 'ভোট দিন',
    poll_results: 'ফলাফল দেখুন',
    top_categories: 'টপ ক্যাটাগরি',
    categories_section: 'বিভাগ সমূহ',
    special_reports: 'বিশেষ প্রতিবেদন',
    live_tv: 'লাইভ টিভি',
    popular_news: 'জনপ্রিয় খবর',
    newsletter_heading: 'গুরুত্বপূর্ণ খবর',
    newsletter_subheading: 'সবার আগে পেতে',
    newsletter_desc: 'সাবস্ক্রাইব করুন আমাদের নিউজলেটার',
    email_placeholder: 'আপনার ইমেইল লিখুন',
    subscribe: 'সাবস্ক্রাইব করুন',
    terms_agree: 'আমি শর্তাবলী এবং গোপনীয়তা নীতি অনুসরণ করছি',
    main_news: 'প্রধান খবর',
    watch_now: 'এখনই দেখুন',
    news_count_suffix: 'খবর',
  },

  // Sections
  sections: {
    explore_more_politics: 'আরও রাজনীতি সংবাদ পড়ুন',
    explore_more_sports: 'আরও খেলার সংবাদ পড়ুন',
    explore_more_entertainment: 'আরও বিনোদন সংবাদ পড়ুন',
    all_sports_news: 'সব খেলার খবর',
    all_entertainment_news: 'বিনোদনের খবর',
    all_regional_news: 'সমস্ত আঞ্চলিক খবর',
    entertainment_title: 'বিনোদন',
    entertainment_sub: 'বলিউড, টলিউড ও ওটিটি দুনিয়া',
    regional_title: 'আঞ্চলিক সংবাদ',
    regional_sub: 'পশ্চিম বর্ধমান, আসানসোল ও দুর্গাপুরের সর্বশেষ তথ্য',
    business_title: 'অর্থনীতি ও বাণিজ্য',
    view_all_arrow: 'সব দেখুন →',
  },

  // Article Page
  article: {
    published: 'প্রকাশিত',
    updated: 'আপডেটেড',
    author: 'লেখক',
    category: 'বিভাগ',
    tags: 'ট্যাগ',
    share: 'শেয়ার করুন',
    bookmark: 'বুকমার্ক',
    comments: 'মন্তব্যসমূহ',
    related: 'সম্পর্কিত খবর',
    ai_summary: 'এআই সারাংশ',
    listen: 'শুনুন',
    reactions: 'প্রতিক্রিয়া',
    report: 'রিপোর্ট করুন',
  },

  // Live TV
  live: {
    title: 'লাইভ টিভি',
    subtitle: 'নির্ভীক বাংলা লাইভ',
    tagline: 'সত্য প্রকাশে নির্ভীক, জনগণের কণ্ঠস্বর',
    offline: 'বর্তমানে সম্প্রচার বন্ধ রয়েছে।',
    upcoming: 'আসন্ন লাইভ',
    starting_soon: 'শীঘ্রই শুরু হচ্ছে...',
    default_desc: 'দেশ-বিদেশের সর্বশেষ ও গুরুত্বপূর্ণ খবর জানুন আমাদের লাইভ সম্প্রচারে।',
    recent_videos: 'লাইভে সম্প্রচারিত ভিডিও',
    schedule: 'সম্প্রচার সূচি',
    watch_live: 'লাইভ দেখুন',
  },

  // Footer
  footer: {
    about: 'আমাদের সম্পর্কে',
    about_text: 'নির্ভীক বাংলা - পশ্চিমবঙ্গের অন্যতম নির্ভরযোগ্য ডিজিটাল নিউজ পোর্টাল। আমরা নিরপেক্ষ, সাহসী এবং জনমুখী সাংবাদিকতায় প্রতিশ্রুতিবদ্ধ।',
    contact: 'যোগাযোগ',
    quick_links: 'দ্রুত লিঙ্ক',
    follow_us: 'আমাদের অনুসরণ করুন',
    newsletter: 'নিউজলেটার',
    newsletter_text: 'প্রতিদিনের গুরুত্বপূর্ণ খবর আপনার ইমেইলে পান',
    subscribe: 'সাবস্ক্রাইব',
    copyright: '© ২০২৬ নির্ভীক বাংলা। সর্বস্বত্ব সংরক্ষিত।',
    privacy: 'গোপনীয়তা নীতি',
    terms: 'ব্যবহারের শর্তাবলী',
    advertise: 'বিজ্ঞাপন দিন',
  },

  // Common
  category_page: {
    all_news: 'সব খবর',
    more: 'আরও',
    latest: 'সর্বশেষ',
    popular: 'জনপ্রিয়',
    top_story: 'শীর্ষ খবর',
    latest_news: 'সর্বশেষ খবর',
    view_all: 'সব দেখুন',
    view_more: 'আরও দেখুন',
    follow: 'ফলো করুন',
    following: 'ফলো করছেন',
    followers: 'ফলোয়ার্স',
    load_more: 'আরও খবর দেখুন',
    loading_more: 'আরও খবর লোড হচ্ছে...',
    staff_reporter: 'নিজস্ব সংবাদদাতা',
    inbox_heading: 'আপনার ইনবক্সে সেরা খবর',
    inbox_desc: 'দৈনিক গুরুত্বপূর্ণ সব আপডেট পেতে সাবস্ক্রাইব করুন',
    email_placeholder: 'আপনার ইমেইল দিন',
    subscribe: 'সাবস্ক্রাইব করুন',
    subscribed_success: 'সাবস্ক্রাইব করার জন্য আপনাকে ধন্যবাদ!',
  },
  sections: {
    politics_title: 'রাজনীতি',
    politics_sub: 'দেশ ও রাজ্যের রাজনৈতিক আপডেট',
    all_politics_news: 'সব রাজনীতি খবর',
    top_politics: 'শীর্ষ রাজনীতি',
    sports_title: 'খেলাধুলা',
    sports_sub: 'ক্রিকেট, ফুটবল, আইপিএল ও খেলার তাজা খবর',
    all_sports_news: 'সব খেলার খবর',
    sports_badge: 'নির্ভীক স্পোর্টস',
    entertainment_title: 'বিনোদন',
    entertainment_sub: 'বলিউড, টলিউড ও ওটিটি দুনিয়া',
    all_entertainment_news: 'বিনোদনের খবর',
    special_report_badge: 'বিশেষ প্রতিবেদন',
    regional_title: 'আঞ্চলিক সংবাদ',
    regional_sub: 'পশ্চিম বর্ধমান, আসানসোল ও দুর্গাপুরের সর্বশেষ তথ্য',
    all_regional_news: 'সমস্ত আঞ্চলিক খবর',
    regional_update: 'আঞ্চলীক আপডেট',
    tech_title: 'প্রযুক্তি ও গ্যাজেট',
    business_title: 'অর্থনীতি ও বাণিজ্য',
    view_all_arrow: 'সব দেখুন →',
  },
  common: {
    loading: 'লোড হচ্ছে...',
    error: 'একটি ত্রুটি ঘটেছে',
    retry: 'আবার চেষ্টা করুন',
    no_results: 'কোন ফলাফল পাওয়া যায়নি',
    ago: 'আগে',
    minutes: 'মিনিট',
    hours: 'ঘণ্টা',
    days: 'দিন',
  }
};

module.exports = bn;
